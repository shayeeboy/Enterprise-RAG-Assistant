...
| Faithfulness (LLM-judge) | answers strictly derivable from retrieved context | **94%** (hallucination 6%, over answered) | `npm run eval:judge` |
| Answer correctness (LLM-judge, 0–5) | mean score vs human-validated golden answers | **3.94 / 5** (over answered) | `npm run eval:judge` |
| Semantic Hit@5 (LLM-judge) | top chunks semantically contain the answer (not keyword match) | **88% (15/17)** | `npm run eval:judge` |
...
previously needed human judgement. Reference run (answerability gate on, the
ground truth's tune/jazz relabel applied, and the retrieval + generation fixes
below — wider rerank funnel, raw+rewrite union, `TOP_K` 8, few-shot exemplar):
**faithfulness 94%, correctness 3.94 / 5, semantic Hit@5 88%, refusal 100%**
(over the 17 answered of 19 answerable; 1 refused as thin-coverage, and 1 —
`parallel-sets` — excluded because the judge model couldn't return valid JSON
for it, not a quality miss). Correctness cleared its gate this run (2.63 → 3.94,
the target of the `TOP_K` + few-shot change) while Hit@5 held. **Single-run
caveat:** these are one run's numbers — correctness in particular varies run to
run, and Hit@5's 88% excludes the one skipped question (with it counted it'd be
~83–89%), so a median of N is still the goal; CI gates on regression floors, not
this single reading. Still N/A: there is no response cache, so "cache hit rate"
doesn't apply.
...
| [**Phase 4 — LLM-Judge evaluation**](#phase-4-llm-judge-evaluation) | deterministic judge scores faithfulness, answer-correctness, and semantic Hit@5 vs golden answers | closes the human-judgment gaps; faithfulness 94%, correctness 3.94/5, refusal 100%, $0 |
...
runs the eval chain weekly and regenerates [`eval/summary.json`](eval/summary.json) —
the block `/snapshot` reports to the Studio — via `npm run eval:summary`
([`scripts/build-eval-summary.js`](scripts/build-eval-summary.js)), which folds the
judged metrics (`eval:judge`) + Keyword Hit@5 (`eval`) and computes
`knowledgeFreshnessDays` live from the newest document in the KB (never hardcoded).
`summary.json` is **generated, never hand-edited**. Guarded to skip cleanly unless
`DATABASE_URL` + `JUDGE_API_KEY` are set (a fork runs it at $0). The *operational*
half of `/snapshot` (latency, grounded rate, cost/query, volume) is already live from
the observability store per request; the refreshed summary reaches the live container
on its next deploy (`gcloud run deploy rag-assistant --source . --region us-central1`).
...
- **Ground truth:** **19** golden answers written from the corpus itself
  ([`eval/ground_truth.json`](eval/ground_truth.json)) plus **3** out-of-scope
  cases, including an adversarial near-miss (piano-adjacent but genuinely
  uncovered — the piano's history / construction). A corpus audit moved two
  earlier "near-misses" (self-tuning, jazz improvisation) into the answerable
  set once the source text was found to cover them — see the scope-correction
  note in the acceptance table.

### Gaps addressed

| Honest gap (before) | Action taken (Phase 4) | Outcome |
|---|---|---|
| Hallucination rate + answer correctness were **not measured** — they needed human judgement | Deterministic LLM-judge scores faithfulness (answer ⊂ context) and correctness (0–5 vs golden answers) | Now measured every run: **faithfulness 94%, mean correctness 3.94 / 5** (over answered) |
| Hit@5 was a **keyword-match proxy** (reported 100%) that can't tell real substance from a coincidental word | Judge re-scores Hit@5 **semantically** against the golden answer | Honest **semantic Hit@5 88% (15/17)** — the proxy was over-optimistic |
...
Generator `openai/gpt-oss-120b`, judge `openai/gpt-oss-20b` (temp 0),
over 19 answerable + 3 out-of-scope questions:

| Metric | Result | Near-term goal | Met? |
|---|---|---|---|
| Faithfulness (no hallucination) | **94%** (hallucination 6%) | ≥ 85% | ✅ met |
| Mean answer correctness | **3.94 / 5** | ≥ 3.6 | ✅ met |
| Semantic Hit@5 | **88%** (15/17) | ≥ 90% | ✗ in progress (just under) |
| Out-of-scope refusal | **100%** (3/3) | 100% | ✅ met |
| Answered (coverage) | **17/19** answerable — 1 thin-coverage refused (jazz-improv), 1 skipped (judge could not return valid JSON for `parallel-sets`) | — | coverage, not graded |

_Answer-quality metrics are over the **answered** questions: a question the
pipeline refuses has no answer to grade, so it is counted as coverage rather than
graded as a wrong answer._

**Did adding the answerability gate tank quality? No — that was a harness bug.**
An earlier version of this section reported faithfulness 59% / correctness 2.71
and blamed the gate. The real cause: when the gate refuses a thin-coverage
answerable question (*"best way to practice scales"*, *"how to warm up"*), the
eval was feeding the **refusal text** to the judge alongside the retrieved
chunks — so the judge flagged the honest "I couldn't find it in the knowledge
base" as **both** a hallucination (the chunks *do* mention the topic) **and**
correctness 0. That double-penalised a *correct* refusal. Fixed: refused
answerable questions are now counted as **coverage** and not graded as answers.
Re-aggregated under the corrected methodology:

| Metric | Pre-gate (17 Q) | With gate, **fixed** (answered) | Δ |
|---|---|---|---|
| Faithfulness | 76% | **67%** | −9 (single-run judge variance) |
| Correctness | 3.06 | **3.07** | ~flat |
| Semantic Hit@5 | 71% | **73%** | ~flat |
| Out-of-scope refusal | 40% | **80%** | **+40 — the gate's win** |

So the gate roughly **doubled** out-of-scope refusal while answer quality on the
questions it *does* answer held steady. Two genuinely thin-coverage questions are
now refused — a **coverage** gap (the corpus barely covers scales / warm-up),
addressed by expanding the KB, not by weakening the gate. The residual
faithfulness movement is single-run judge variance, which is why CI gates on
**regression floors** (hallucination ≤ 34%, correctness ≥ 2.7, Hit@5 ≥ 75%,
refusal = 100%) rather than the aspirational goals — and why the nightly
auto-refresh is now paused, so one noisy run can't rewrite the headline.
...
|---|---|---|---|---|
| Faithfulness (no hallucination) | 94% | ≥ 85% | ≥ 95% | ✅ **shipped + goal met:** temperature-0 generation; a citation-grounding trim (`ENFORCE_CITATIONS`); and a local **NLI entailment filter** (`ENFORCE_ENTAILMENT`, opt-in) that drops answer sentences the retrieved context doesn't entail. (94% this run, up from 81%; the few-shot exemplar's "cite every point" also tightens grounding — though single-run variance means a median would confirm.) **Next:** tune the NLI threshold against the eval — strict sentence-level NLI is aggressive on synthesized sentences, so it's off by default until measured; harden the grounding prompt ("state only what a source explicitly says — never infer"); report the median of N runs to damp variance. |
| Answer correctness (0–5) | 3.94 | ≥ 3.6 | ≥ 4.2 | ✅ **shipped + goal met (2.63 → 3.94):** added a **few-shot exemplar** of a thorough, fully-cited answer (covers *every* relevant source point) and raised **`TOP_K` 6 → 8** so more golden-answer substance reaches the generator. This was the failing gate after the retrieval changes moved the weak spot from recall to answer precision; the fix targeted precision without giving back Hit@5. **Caveat:** single run — correctness is the noisiest metric (it has ranged 2.63–3.94), so a median of N would confirm. **Next:** add full **multi-query** retrieval and close the corpus gaps below to reach the ≥ 4.2 stretch. |
| Semantic Hit@5 | 88% | ≥ 90% | ≥ 95% | **63% → 81% → 88% (15/17)** as the retrieval fixes landed — now just under the ≥ 90% goal, well above the ≥ 75% CI floor. ✅ **shipped (targeting this):** widened the rerank funnel (`RERANK_INPUT` 10 → 20, `HYBRID_CANDIDATES` 20 → 30) so a definition chunk ranked outside the top 10 still reaches the reranker; and **raw + rewrite union retrieval** (`UNION_RAW_REWRITE`) so a drifting rewrite can't drop a chunk the original finds — the two dominant miss modes (definition split from its chunk; retrieval instability). Note this run's 88% excludes `parallel-sets` (a known definition-split miss) which the judge skipped; counting it would be ~83–89%. **Next:** **expand the knowledge base** for genuine coverage gaps (an absolute-beginner primer, a dedicated scales/technique reference); add full **multi-query** fan-out; trial a larger embedding model or a fine-tuned reranker. |
...
| Issue | Resolution / lesson |
|---|---|
| The keyword eval reported a reassuring Hit@5 100% | A semantic LLM-judge showed the honest picture (currently Hit@5 88%, faithfulness 94%, correctness 3.94/5). A proxy that only checks for a keyword can't see whether the *answer* is right — measure the answer, not just the retrieval. |
| The eval graded a *refusal* as a hallucination | After the answerability gate shipped, the "Gaps Addressed" numbers looked like they were tanking (faithfulness 76%→59%, correctness 3.06→2.71). Root cause was **not** the model or the gate — it was the harness feeding a gate's refusal text ("I couldn't find it in the KB") to the faithfulness/correctness judge *with the retrieved chunks attached*, so the judge scored the honest refusal as **both** a hallucination **and** correctness 0. Fixed by counting refused answerable questions as **coverage**, not grading them as answers — which recovered correctness to 3.07 and revealed the gate's real effect: out-of-scope refusal **doubled (40%→80%)** with answer quality flat. Also paused the nightly auto-refresh so a single noisy run couldn't keep rewriting the headline. Lesson: when a new component changes behavior, check that the *evaluation* still measures the right thing before trusting the trend. |
...
| Adversarial near-misses defeat a score threshold | A genuine near-miss (e.g. the piano's history/construction) scores **as high on the reranker as valid questions** (0.97–1.0) — its chunks are topically piano-related — so no relevance-floor value separates it from thin-coverage in-scope questions (0.04–0.29). Fixed with an **answerability gate**: a focused LLM YES/NO on whether the chunks actually answer *this* question, kept separate from the "be helpful" generation step (which kept rationalizing an answer). Also: validate on the **real pipeline path** — an early test skipped query-rewrite and mis-measured the gate, because rewrite changes which chunks it sees. Lesson: topical relevance ≠ answerability; that judgement needs an LLM, not a score. |
| A "hallucination" was really a mislabeled test | Two "out-of-scope near-misses" (`tune-piano`, `jazz-improv`) kept getting answered and scored as failures — the tempting fix was to make the gate stricter until they refused. Probing the retrieval first (retrieve → rerank → gate in isolation, one tiny gate call each, no generation) showed the gate was reading a real **tuning chapter** (#178/#194, Reblitz reference) and explicit **jazz/improvisation** guidance (#128): the corpus genuinely covers both. The gate was *right*; the eval's ground truth was wrong. Fixed by correcting the labels, not the gate — forcing refusals would have hidden real capability and gamed the metric. Lesson: before hardening a model against a "wrong" answer, verify the ground truth against the source — and probe the cheap inner stage in isolation rather than burning full eval runs. |
