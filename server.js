...
// cost/query, volume) AND knowledge freshness are LIVE (freshness is computed
// per request from the newest ingested document); the judged quality metrics
// come from eval/summary.json, GENERATED by `npm run eval:summary` from the eval
// run (never hand-edited). Shape matches the Studio's RagHealthSnapshot contract.
app.get("/snapshot", async (_req, res) => {
  ...
  // Knowledge freshness is a LIVE property of the KB (days since the newest
  // document was ingested), so compute it per request rather than serving the
  // value baked into summary.json at eval time — that way it can't go stale
  // between deploys. Falls back to the committed value, then null, on DB error.
  out.knowledgeFreshnessDays = summary.knowledgeFreshnessDays ?? null;
  try {
    const fr = await pool.query("SELECT max(created_at) AS latest FROM documents");
    const latest = fr.rows[0] && fr.rows[0].latest;
    if (latest) out.knowledgeFreshnessDays = Math.max(0, Math.floor((Date.now() - new Date(latest).getTime()) / 86_400_000));
  } catch (e) {
    out.provenance.freshnessError = e.message;
  }
